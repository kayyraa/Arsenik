...
Api.SimulationResetButton.addEventListener("click", () => {
	Api.Playground.innerHTML = "";
	Frame = 0;
});

function Loop() {
	Api.SimulationFrameLabel.innerHTML = `Frame: ${Frame}`;
	...
	Particles.forEach(Particle => {
		if (!Particle.Velocity) Particle.Velocity = { X: 0, Y: 0 };
	});
	...
	Particles.forEach((ParticleA, IndexA) => {
		...
		Particles.forEach((ParticleB, IndexB) => {
			...
			let BondStrength = 1;
			let ConnectionPadding = 0;
			if (ConnectedLine) BondStrength = Math.max(Math.abs(parseFloat(ConnectedLine.style.width)) / 100, 1); ConnectionPadding = 16;

			if (Distance <= (RadiusA + RadiusB) / 2 + ConnectionPadding) {
