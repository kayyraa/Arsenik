...
function Loop() {
	...
	if (ElapsedTime >= 125) {
		...
		Api.SimulationFpsLabel.innerHTML = `Fps: ${Fps}`;
		FrameCount = 0;
	...
	document.querySelectorAll("div.Line").forEach(Line => {
		...
		if (window.View.DisplayConnectionData.LengthData) {
			Line.querySelectorAll("span")[0] 
				? Line.querySelectorAll("span")[0].innerHTML = parseFloat(Line.style.width).toFixed(1) 
				: "";
		} else {
			Line.querySelectorAll("span")[0] 
				? Line.querySelectorAll("span")[0].innerHTML = "" 
				: "";
		}
		...
		const NewLine = new Api.Line(undefined, [
			...
			Gradient: window.View.ConnectionStyle.Gradient 
				? new Api.Gradient(new Api.Matrix([
				...
				: undefined,
			ColorRay: window.View.ConnectionStyle.Strip 
				? new Api.Matrix([
					Api.Playground.querySelectorAll(`div:not(.Line)[uid="${Connection[0]}"]`)[0].style.backgroundColor,
					Api.Playground.querySelectorAll(`div:not(.Line)[uid="${Connection[1]}"]`)[0].style.backgroundColor,
				]) 
		...
		NewLine.innerHTML = window.View.DisplayConnectionData.LengthData 
			? parseFloat(NewLine.style.transform.replace("rotate(", "").replace("deg)", "")).toFixed(1) 
			: "";
		Line.remove();
	});
